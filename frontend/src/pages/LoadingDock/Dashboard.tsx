...
export function LoadingDockDashboard() {
  ...
  const handleComplete = async (order: Order) => {
    ...
    const ok = await run(
      ...
      'Loading recorded and waybill issued.',
    );
    if (ok) resetDialog();
  };
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="mx-auto max-w-6xl p-8">
        ...
        {currentlyLoading.length > 0 && (
          <section>
            <h2 className="mb-1 text-xl font-semibold">Currently loading</h2>
            <p className="mb-4 text-sm text-gray-600">
              Record the quantity dispensed to issue the waybill and close the order.
            </p>
            <div className="grid gap-4">
              {currentlyLoading.map((order) => (
                <Card key={order.id}>
                  <CardHeader>
                    <div className="flex items-start justify-between">
                      <div>
                        <CardTitle className="text-lg">{order.npa_reference_number}</CardTitle>
                        <p className="text-sm text-gray-500">
                          {customerName(order)} · {order.verified_truck_number ?? order.truck_number}
                        </p>
                      </div>
                      <Badge className={`border ${STATUS_BADGE[order.status]}`}>
                        {STATUS_LABEL[order.status]}
                  ...
                  <CardContent>
                    ...
                    >
                      <DialogTrigger asChild>
                        <Button>Record quantity &amp; issue waybill</Button>
                      </DialogTrigger>
                      <DialogContent>
                        <DialogHeader>
                          <DialogTitle>Complete loading</DialogTitle>
                          <DialogDescription>{order.npa_reference_number}</DialogDescription>
                        </DialogHeader>
                        <div className="space-y-4">
                          <div className="rounded border bg-gray-50 p-3 text-sm">
                          ...
                          <div className="space-y-2">
                            ...
                            />
                            <p className="text-xs text-gray-500">
                              Cannot exceed the approved volume.
                            </p>
                          </div>
                          <Button
                            className="w-full"
